...
const ViewApplications = () => {
  ...
  useEffect(() => {
    if (companyToken) {
      fetchCompanyJobApplicants() //fetch company job applicants data when job list is loaded
    }
  }, [companyToken])
                  ...
                  {applicant.status === 'Pending'
                    ? <div className='relative inline-block text-left group'>
                      ...
                      <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                        <button onClick={() => changeJobApplicationStatus(applicant._id, 'Accepted')} className='block w-full text-left px-4 py-2 text-blue-500 hover:bg-blue-50 rounded'>Accept</button>
                        <button onClick={() => changeJobApplicationStatus(applicant._id, 'Rejected')} className='block w-full text-left px-4 py-2 text-red-500 hover:bg-red-50 rounded'>Reject</button>
                      </div>
                    </div>
